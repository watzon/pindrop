...
final class AIEnhancementServiceTests: XCTestCase {
    ...
    func testEnhanceWithoutAPIKeyDoesNotSetAuthorizationHeader() async throws {
        mockSession.mockData = """
        {
            "choices": [{
                "message": {
    ...
    func testGenerateNoteMetadataWithoutAPIKeyDoesNotSetAuthorizationHeader() async throws {
        mockSession.mockData = """
        {
            "choices": [{
                "message": {
                    "content": "{\\"title\\":\\"Weekly Sync Notes\\",\\"tags\\":[\\"meeting\\",\\"planning\\",\\"team\\"]}"
                }
            }]
        }
        """.data(using: .utf8)
        ...
        let metadata = try await service.generateNoteMetadata(
            content: "weekly team sync action items",
            apiEndpoint: "http://localhost:11434/v1/chat/completions",
            apiKey: nil,
            model: "llama3.2"
        )

        XCTAssertEqual(metadata.title, "Weekly Sync Notes")
        XCTAssertEqual(metadata.tags, ["meeting", "planning", "team"])
        XCTAssertNil(mockSession.lastRequest?.value(forHTTPHeaderField: "Authorization"))
    }

    func testOllamaProviderConfiguration() {
        XCTAssertEqual(AIProvider.ollama.defaultEndpoint, "http://localhost:11434/v1/chat/completions")
        XCTAssertFalse(AIProvider.ollama.requiresAPIKey)
        XCTAssertTrue(AIProvider.ollama.isImplemented)
        XCTAssertEqual(AIProvider.ollama.apiKeyPlaceholder, "Not required for local Ollama")
    }
}
