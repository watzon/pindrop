...
struct AIEnhancementSettingsView: View {
    ...
    private var providerConfigContent: some View {
        if !selectedProvider.isImplemented {
            ...
            VStack(spacing: 16) {
                ...
                if selectedProvider == .openrouter || selectedProvider == .ollama {
                    modelPicker
                }
    ...
    private func loadCredentials() {
        ...
        if let endpoint = settings.apiEndpoint {
            ...
            } else if endpoint.contains("localhost:11434") || endpoint.contains("127.0.0.1:11434") || endpoint.contains("ollama") {
    ...
    private func saveCredentials() {
        ...
        do {
            ...
            } else if selectedProvider == .openrouter || selectedProvider == .ollama {
                settings.aiModel = resolvedModel()
            } else {
    ...
    private func resolvedModel() -> String {
        ...
        if recommended.contains(selectedModel) {
            return selectedModel
        }

        return recommended.first ?? selectedModel
    }
}
