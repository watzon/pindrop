...
enum AIProvider: String, CaseIterable, Identifiable {
   ...
   var isImplemented: Bool {
      switch self {
      case .openai, .ollama, .openrouter, .custom: return true
...
struct AIEnhancementStepView: View {
   ...
   private var emptyModelsMessage: String {
      ...
      if selectedProvider == .ollama {
         return "Ensure Ollama is running locally to load models."
      }
      if modelError != nil {
   ...
   private func loadModelsIfNeeded(for provider: AIProvider, forceRefresh: Bool = false) async {
      guard provider == .openrouter || provider == .openai || provider == .ollama else { return }
      modelError = nil
      ...
      case .openrouter where !selectedModel.contains("/"):
         selectedModel = defaultModelIdentifier(for: provider)
      case .ollama where selectedModel.contains("/"):
         selectedModel = defaultModelIdentifier(for: provider)
      default:
         break
      }
   ...
   private func fetchModels(for provider: AIProvider) async throws -> [AIModelOption] {
      ...
      if provider == .ollama {
         ...
         models = modelsArray.compactMap { entry -> AIModelOption? in
            guard let name = entry["name"] as? String else { return nil }
            return AIModelOption(id: name, name: name)
         }
      } else {
         guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
         ...
         models = dataArray.compactMap { entry -> AIModelOption? in
   ...
   private func cacheURL(for provider: AIProvider) -> URL? {
      ...
      case .ollama:
         return directory.appendingPathComponent("ollama-models.json")
      default:
         return nil
      }
   }
   ...
   private var canContinue: Bool {
      ...
      if selectedProvider.requiresAPIKey && apiKey.isEmpty { return false }
   ...
   private func saveAndContinue() {
      ...
      if selectedProvider.requiresAPIKey {
         ...
         try? settings.deleteAPIKey(for: selectedProvider)
      }
      if selectedProvider == .custom {
